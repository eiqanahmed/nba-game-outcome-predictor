...
4. (RECOMMENDED) Go to `https://cdn.nba.com/static/json/staticData/scheduleLeagueV2.json`, copy everything (Ctrl + A -> Ctrl + C on windows, or command + A -> command + C on mac), create a file called `scheduleLeagueV2.json`, paste what you copied, and store the file in the same directory as `predict.py`. This way even if the api call fails, we can still access the NBA schedule to determine each team's next game.
5. Run `predict.py` to obtain the predictions for each team's next game.
6. (Optional) Run `testing_accuracy.py` to test the accuracy of the model on historical data.
